...
n = 20
names = [f"ship_{i}_{j}" for i in range(n) for j in range(n)]
...
if __name__ == "__main__":
    for n_processes in range(1, 7):
        ...
        with Pool(n_processes) as p:
            p.startmap(process_threshold, [(cv2.imread(INPUT_PATH + name),
                                            name,
                                            THRESH_METHOD) for name in names])
            t = timer() - start
            print(f"Took {t:.4f} seconds with {i} process(es).")
